...
async function renderedRows():Promise<Row[]>{
 ...
 try{
  ...
  const rows=await page.evaluate(()=>{
   ...
   }).filter(Boolean) as {score:number,body:Record<string,string>[]}[];
   candidates.sort((a,b)=>b.score-a.score);
   return candidates[0]?.body??[];
  });
  if(!rows.length)throw new Error("The official timetable page loaded but no timetable rows were rendered");
  return rows;
