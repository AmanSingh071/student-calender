...
function Dashboard({matches,selected,googleAccount,onSetup,onTimetable}:any){
 ...
 // Dashboard must only consider subjects the student has actually selected.
 // selected is the array of selected subject objects passed from the setup state.
 const selectedSubjectKeys=new Set((selected||[]).flatMap((s:any)=>[
   normalize(s.id||""),normalize(s.code||""),normalize(s.name||"")
 ]).filter(Boolean));
 const personalisedMatches=(matches||[]).filter((m:any)=>
   selectedSubjectKeys.has(normalize(m.code||""))||
   selectedSubjectKeys.has(normalize(m.subject||""))
 );
 // Find the nearest real upcoming occurrence among those selected subjects.
 const upcoming:any[]=Array.from(new Map<string,any>(
   personalisedMatches.map(nextOccurrence).filter(Boolean).map((m:any)=>[keyFor(m),m] as [string,any])
 ).values()).sort((a:any,b:any)=>+new Date(a.start)-+new Date(b.start));
