...
export default function Home(){
 ...
 return <main>
  {sync&&<div className="syncOverlay"><div className="syncCard"><Loader2 className="spin" size={34}/><div className="syncLabel">{sync.mode==="importing"?"IMPORTING TO GOOGLE CALENDAR":sync.mode==="reverting"?"REVERTING CALENDAR CHANGES":"CONNECTING GOOGLE"}</div><h2>{sync.mode==="importing"?sync.total>0?`Importing ${sync.current} of ${sync.total} classes…`:"Preparing your timetable…":sync.mode==="reverting"?"Removing Student Calendar events and restoring your setup…":"Opening secure Google sign-in…"}</h2>{sync.mode==="importing"&&<><div className="progressTrack"><span style={{width:sync.total>0?`${Math.max(6,(sync.current/sync.total)*100)}%`:"18%"}}/></div><p>{sync.total>0?`${sync.current} of ${sync.total} events completed`:"Matching your selected subjects with the latest official schedule…"}</p></>}{sync.mode==="reverting"&&<p>{sync.current>0?sync.current+" event(s) removed so far…":"Finding Student Calendar events to remove…"}</p>}</div></div>}
