...
type Match={day?:string;start:string;end:string;code:string;section?:string;teacher:string;subject:string};
...
export default function Home(){
 ...
 async function importCalendar(){
  ...
  try{
   ...
   const events=found.map(m=>({summary:m.subject,description:[m.code,m.section?"Section "+m.section:"",m.teacher].filter(Boolean).join(" · "),sourceKey:[m.code,m.section||"",m.start,m.end,m.teacher].join("|"),start:m.start,end:m.end}));
...
function field(row:any,...names:string[]){for(const n of names){const k=Object.keys(row).find(x=>norm(x)===norm(n));if(k)return row[k]}return undefined}
function rowsFrom(source:any){const out:any[]=[];const walk=(v:any)=>{if(Array.isArray(v))return v.forEach(walk);if(v&&typeof v==="object"){const ks=Object.keys(v).map(norm);if(ks.some(k=>k.includes("CODE")||k.includes("SUBJECT")||k.includes("COURSE"))&&ks.some(k=>k==="DATE"||k.includes("DAY")||k.includes("START")))out.push(v);Object.values(v).forEach(walk)}};if(typeof source==="string"){try{walk(JSON.parse(source))}catch{}}else walk(source);return out}
function subjectFor(row:any){const code=String(field(row,"code","subject code","course code")||""),name=String(field(row,"subject","subject name","course")||"");return subjects.find(s=>norm(code)===norm(s.code)||norm(name)===norm(s.name)||norm(name).includes(norm(s.name))||norm(s.name).includes(norm(name)))}
function findSections(source:any){const map:Record<string,string[]>={};for(const row of rowsFrom(source)){const s=subjectFor(row);const sec=String(field(row,"section")||"").trim().toUpperCase();if(s&&/^[AB]$/.test(sec))map[s.id]=Array.from(new Set([...(map[s.id]||[]),sec]));}return map}
function toIso(date:any,time:any){const d=String(date||"").trim(),t=String(time||"").trim();if(!d)return null;const x=new Date(d.includes("T")?d:(d+"T"+(t||"00:00")+":00+05:30"));return Number.isNaN(x.getTime())?null:x.toISOString()}
function buildMatches(source:any,selected:string[],sections:Record<string,string>){const out:Match[]=[];for(const row of rowsFrom(source)){const s=subjectFor(row);if(!s||!selected.includes(s.id))continue;const sec=String(field(row,"section")||"").trim().toUpperCase();if(sections[s.id]&&sec&&sections[s.id]!==sec)continue;const start=toIso(field(row,"date","class date","event date"),field(row,"start","start time","from"));const end=toIso(field(row,"date","class date","event date"),field(row,"end","end time","to"));if(!start||!end)continue;out.push({start,end,code:s.code,section:sec||undefined,teacher:String(field(row,"teacher","faculty","professor")||s.teacher),subject:s.name});}return out}
