...
function subjectFor(row:any){const code=String(value(row,"code")||""),name=String(value(row,"subject","subject name","course","course name")||"");return subjects.find(s=>norm(code)===norm(s.code)||norm(name)===norm(s.name))}
...
function buildEvents(data:any,selected:string[],sections:Record<string,string>){const out:SyncEvent[]=[];for(const row of rowsFrom(data)){const s=subjectFor(row);if(!s||!selected.includes(s.id))continue;const sec=String(value(row,"section")||"").trim().toUpperCase();if(sections[s.id]&&sec&&sections[s.id]!==sec)continue;const [a,b]=range(row);const rawDate=value(row,"date","class date","event date"),date=dateOnly(rawDate)||nextDay(String(value(row,"day","weekday")||""));if(!date||!a||!b)continue;const start=iso(date,a),end=iso(date,b);if(!start||!end)continue;const recurrence=dateOnly(rawDate)?undefined:["RRULE:FREQ=WEEKLY"];const teacher=String(value(row,"teacher","faculty","professor")||s.teacher),type=String(value(row,"type","event type")||"Class");const sourceKey=[s.code,sec,date,type].join("|");out.push({sourceKey,summary:s.name,description:[s.code,sec?"Section "+sec:"",teacher,type].filter(Boolean).join(" · "),start,end,recurrence})}return out}

async function upsert(profile:any,events:SyncEvent[]){const auth=new google.auth.OAuth2(process.env.GOOGLE_CLIENT_ID,process.env.GOOGLE_CLIENT_SECRET);auth.setCredentials(JSON.parse(decrypt(profile.token)));const cal=google.calendar({version:"v3",auth}),prev=new Map<string,SyncEvent>((profile.events as SyncEvent[]).map((e:SyncEvent)=>[e.sourceKey,e])),next:SyncEvent[]=[];for(const e of events){const old=prev.get(e.sourceKey),body={summary:e.summary,colorId:colorForSubject(e.sourceKey,e.summary),description:e.description,start:{dateTime:e.start},end:{dateTime:e.end},extendedProperties:{private:{studentCalendarSourceKey:e.sourceKey,studentCalendarApp:"student-calendar"}},...(e.recurrence?.length?{recurrence:e.recurrence}:{})};let id=old?.eventId;if(id)await cal.events.update({calendarId:"primary",eventId:id,requestBody:body});else{id=(await cal.events.insert({calendarId:"primary",requestBody:body})).data.id||undefined}next.push({...e,eventId:id})}
 // Do not rely only on profile.events: older buggy versions could have created
 // app-managed events whose IDs were lost from the profile. Query Google Calendar
 // for every event created by this app and remove anything not in this sync result.
 const keep=new Set(next.map(e=>e.sourceKey));let pageToken:string|undefined;
 do{
